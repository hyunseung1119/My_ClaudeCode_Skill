...
이 저장소는 Claude Code의 훅 시스템으로 4대 하네스 미들웨어 패턴을 구현합니다:
...
| Auto-format | `tsc-check.sh` | TypeScript 증분 타입 체크 |
| Auto-format | `ruff-format.sh` | Python ruff lint + format |
| Code Quality | `console-log-warning.sh` | console.log 추가 시 경고 |
...
```
├── CLAUDE.md                     ← 글로벌 지침 (32줄, on-demand 규칙 참조)
├── settings.local.json           ← 설정 (권한, 모델, 9개 훅 배선)
...
| **ALWAYS** | `harness-engineering.md` | 4대 미들웨어 패턴, 추론 예산 배분 |
...
| on-demand | `agents.md` | 23개 에이전트 오케스트레이션 |
| on-demand | `git-workflow.md` | Conventional Commits, PR 워크플로우 |
| on-demand | `hooks.md` | 9개 훅 스크립트 설정 가이드 |
...
| `chatbot-designer` | 대화형 UX 설계 (다이얼로그 플로우, Intent/Entity) |
...
| 카테고리 | 에이전트 | 트리거 |
|----------|----------|--------|
| **핵심** | planner, code-reviewer, tdd-guide, security-reviewer, build-error-resolver, debugger, architect | 기능 요청, 코드 작성, 빌드 실패 등 |
| **도메인** | react-agent, rust-expert, go-reviewer, go-build-resolver, graphql-expert, database-reviewer, vector-db-agent | 해당 언어/도메인 작업 시 |
| **품질** | a11y-reviewer, performance-optimizer, refactor-cleaner, doc-updater, e2e-runner, infrastructure-agent | 품질/인프라 작업 시 |
| **메타** | coordinator, critic-agent, tree-of-thoughts | 복잡한 멀티스텝 작업 시 |
